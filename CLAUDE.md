...
{ id, originalMerchant, mappedMerchant, createdAt, updatedAt } // MerchantMapping (unique per database on originalMerchant; trailing "*" = prefix/wildcard match)
```
...
**Merchant Mappings**: Auto-cleaned merchant name (`transaction.merchant` before any user edit) → user's canonical name, per database, unique on `originalMerchant`. Matching is case-insensitive (so "TARGET" and "Target" from different bank exports resolve to the same mapping) — `resolveMerchantMapping`/`mappingMatchesKey` compare via `strings.EqualFold`/`.toLowerCase()`, and `UpsertMerchantMapping`'s lookup uses `original_merchant = ? COLLATE NOCASE` so re-capturing a differently-cased spelling updates the existing row (refreshing its stored casing) instead of creating a duplicate; the SQL `UNIQUE` index itself is still case-sensitive, but the app never creates a case-variant duplicate through normal use. `originalMerchant` may be an exact key or end in a single trailing `*` for a prefix/wildcard match (e.g. `"IN-N-OUT BURGER*"` → `"In-N-Out"` covers every location); `resolveMerchantMapping`/`mappingMatchesKey` (duplicated in `merchant_mapping.go` and `helpers.js`, must stay in sync) resolve which mapping wins for a given key — exact always beats wildcard, longest-prefix wildcard wins otherwise. `isValidMerchantMappingPattern` (also duplicated Go/JS) enforces the trailing-only wildcard syntax. Captured automatically on Review-page save whenever the merchant field is changed (`review.js` `saveTransaction`/`captureMerchantMapping`, always as an exact mapping) and pre-filled (with a hint) for not-yet-reviewed transactions whose merchant resolves to a saved mapping (`findMappedMerchant`). Every create/update — whether from Review auto-capture or the "Manage Merchant Mappings" dashboard modal — calls `reconcileMerchantMappings`, which re-resolves **every** transaction in the database (reviewed or not) against the full current mapping set and rewrites `merchant` wherever it disagrees (`CleanMerchantName` is a Go port of `helpers.js`'s `cleanMerchantName` and must stay in sync with it). Deleting a mapping does not revert transactions. The "Scan Existing Transactions" action (`ScanMerchantMappingCandidates`) backfills mappings from history and is exact-key-only (no wildcard auto-suggestion), grouping case-insensitively so e.g. "TARGET" and "Target" clean keys land in one group (a pure case change is still a real rename to capture, so the per-transaction "was this edited" check itself stays case-sensitive): transactions where `merchant` already differs from the recomputed clean key, and isn't already explained by an existing mapping, indicate a past rename — one distinct renamed spelling per key auto-migrates, 2+ distinct spellings (e.g. a typo) surface as a conflict in the modal for the user to resolve by picking/typing the canonical spelling, which then applies to every variant sharing that key.
